...
export default function ScoreBoard({ history, aggregated, onViewReport }: ScoreBoardProps) {
    return (
        <Card className="animate-in zoom-in-95 duration-500 bg-card border-border shadow-2xl overflow-hidden mb-12">
            ...
            <CardHeader className="text-center pb-8 pt-10 relative">
                ...
                <p className="text-muted-foreground text-sm font-medium">Excellent work! Here is the summary of your performance.</p>

                {aggregated && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                        <div className="p-4 bg-background/70 rounded-xl border border-border/60">
                            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Overall Avg</p>
                            <p className="text-2xl font-black text-primary">{aggregated.overallAverage.toFixed(1)}</p>
                        </div>
                        <div className="p-4 bg-background/70 rounded-xl border border-border/60">
                            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Technical</p>
                            <p className="text-2xl font-black text-emerald-500">{aggregated.averageTechnical.toFixed(1)}</p>
                        </div>
                        <div className="p-4 bg-background/70 rounded-xl border border-border/60">
                            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Problem Solving</p>
                            <p className="text-2xl font-black text-amber-500">{aggregated.averageProblemSolving.toFixed(1)}</p>
                        </div>
                        <div className="p-4 bg-background/70 rounded-xl border border-border/60">
                            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Communication</p>
            ...
            <CardContent className="px-8 pb-10 relative">
                <div className="space-y-3">
                    {history.map((entry, i) => {
                        ...
                        return (
                            <div key={i} className="flex items-center justify-between p-4 bg-background/60 rounded-xl border border-border/50 hover:bg-accent/60 transition-colors group">
                                <div className="flex items-center gap-4">
                                    <div className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center text-xs font-bold text-muted-foreground">
                                        Q{entry.questionNumber}
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wider">{entry.question.topic}</p>
                                        <p className="text-sm font-bold text-foreground/80 group-hover:text-foreground transition-colors">Evaluation Complete</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="text-right">
                                        <p className="text-lg font-black text-foreground">{score}/10</p>
                ...
                <div className="mt-10">
                    ...
                    <p className="text-center text-muted-foreground text-[10px] mt-4 font-bold uppercase tracking-widest">
